...
<div class="app">
  ...
  <aside class="sidebar">
    ...
    <div class="sidebar-logo">
      ...
      <div class="logo-text">
        <span class="logo-name">StudentLancer</span>
      ...
      <div class="sidebar-section">
        ...
        <ul class="conv-list">
          <li class="conv-item active">
            <svg class="conv-icon" width="13" height="13" viewBox="0 0 24 24" fill="none"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z" stroke="currentColor" stroke-width="2" stroke-linejoin="round"/></svg>
            <span class="conv-title">Wallet Help</span>
            <span class="conv-dot"></span>
          </li>
          <li class="conv-item" onclick="loadConv(this,'How does escrow work on StudentLancer?')">
            <svg class="conv-icon" width="13" height="13" viewBox="0 0 24 24" fill="none"><rect x="3" y="11" width="18" height="11" rx="2" stroke="currentColor" stroke-width="2"/><path d="M7 11V7a5 5 0 0110 0v4" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>
            <span class="conv-title">Escrow Questions</span>
          </li>
          <li class="conv-item" onclick="loadConv(this,'How are AI recommendations calculated?')">
            <svg class="conv-icon" width="13" height="13" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="3" stroke="currentColor" stroke-width="2"/><path d="M12 1v4M12 19v4M4.22 4.22l2.83 2.83M16.95 16.95l2.83 2.83M1 12h4M19 12h4M4.22 19.78l2.83-2.83M16.95 7.05l2.83-2.83" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>
            <span class="conv-title">AI Recommendations</span>
          </li>
          <li class="conv-item" onclick="loadConv(this,'How do withdrawals work on StudentLancer?')">
            <svg class="conv-icon" width="13" height="13" viewBox="0 0 24 24" fill="none"><line x1="12" y1="1" x2="12" y2="23" stroke="currentColor" stroke-width="2" stroke-linecap="round"/><path d="M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>
            <span class="conv-title">Withdrawal Support</span>
          </li>
          <li class="conv-item" onclick="loadConv(this,'Check the status of my job applications.')">
            <svg class="conv-icon" width="13" height="13" viewBox="0 0 24 24" fill="none"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z" stroke="currentColor" stroke-width="2" stroke-linejoin="round"/><polyline points="14 2 14 8 20 8" stroke="currentColor" stroke-width="2"/></svg>
            <span class="conv-title">Job Applications</span>
          </li>
        </ul>
      </div>
    ...
    <div class="sidebar-profile">
      <div class="profile-avatar">KA</div>
      <div class="profile-info">
        <div class="profile-name">Kofi Asante</div>
        <div class="profile-role">Student · KNUST</div>
      </div>
    ...
    <header class="chat-header">
      ...
      <div class="header-actions">
        ...
        <button class="header-action-btn" title="Clear Chat" onclick="clearChat()">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><polyline points="3 6 5 6 21 6" stroke="currentColor" stroke-width="2" stroke-linecap="round"/><path d="M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6M10 11v6M14 11v6" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>
        </button>
        <button class="header-action-btn" title="Expand" onclick="document.documentElement.requestFullscreen?.()">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M8 3H5a2 2 0 00-2 2v3M21 8V5a2 2 0 00-2-2h-3M3 16v3a2 2 0 002 2h3M16 21h3a2 2 0 002-2v-3" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
        </button>
      </div>
    </header>
    ...
    <div class="chat-input-area">
      <div class="quick-chips-row">
        <button class="quick-chip" onclick="sendSuggestion('What wallet features does StudentLancer offer?')">
          <svg width="11" height="11" viewBox="0 0 24 24" fill="none"><rect x="1" y="4" width="22" height="16" rx="2" stroke="currentColor" stroke-width="2"/><path d="M1 10h22" stroke="currentColor" stroke-width="2"/></svg>
          Wallet
        </button>
        ...
        <button class="quick-chip" onclick="sendSuggestion('How are AI recommendations calculated?')">
          <svg width="11" height="11" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="3" stroke="currentColor" stroke-width="2"/><path d="M12 1v4M12 19v4" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>
          Recommendations
        </button>
        <button class="quick-chip" onclick="sendSuggestion('How does the dispute resolution process work?')">
          <svg width="11" height="11" viewBox="0 0 24 24" fill="none"><path d="M18 20V10M12 20V4M6 20v-6" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>
          Disputes
        </button>
        <button class="quick-chip" onclick="sendSuggestion('How do payments work on StudentLancer?')">
          <svg width="11" height="11" viewBox="0 0 24 24" fill="none"><line x1="12" y1="1" x2="12" y2="23" stroke="currentColor" stroke-width="2" stroke-linecap="round"/><path d="M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>
          Payments
        </button>
      </div>
      ...
      <div class="input-box" id="inputBox">
        ...
        <div class="input-right">
          ...
          <button class="input-icon-btn voice-btn" title="Voice input (coming soon)">
            <svg width="17" height="17" viewBox="0 0 24 24" fill="none"><path d="M12 1a3 3 0 00-3 3v8a3 3 0 006 0V4a3 3 0 00-3-3z" stroke="currentColor" stroke-width="2"/><path d="M19 10v2a7 7 0 01-14 0v-2M12 19v4M8 23h8" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>
          </button>
...
function renderMarkdown(raw) {
  ...
  let out='', inUL=false, inOL=false, inCode=false, codeBuf=[], inTable=false, tableBuf=[];
  ...
  const closeTable=()=>{
    ...
    out+='</table></div>'; tableBuf=[];
  };
  for(let i=0;i<lines.length;i++){
    const line=lines[i];
    if(line.startsWith('```')){
      if(!inCode){closeUL();closeOL();closeTable();inCode=true;codeBuf=[];}
      else{inCode=false;out+=`<pre class="code-block"><code>${codeBuf.join('\n')}</code></pre>`;codeBuf=[];}
      continue;
    }
    if(inCode){codeBuf.push(line.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;'));continue;}
    if(line.startsWith('|')){
      closeUL();closeOL();inTable=true;tableBuf.push(line);
      if(i+1>=lines.length||!lines[i+1].startsWith('|'))closeTable();
      continue;
    }
    if(!line.startsWith('- ')&&!/^\d+\./.test(line)){closeUL();closeOL();}
...
function scrollBottom(){const c=document.getElementById('chatMessages');c.scrollTo({top:c.scrollHeight,behavior:'smooth'});}
...
function addUserMessage(text){
  ...
  el.innerHTML=`<div class="msg-bubble msg-bubble--user"><p>${text.replace(/</g,'&lt;')}</p></div><span class="msg-time">${getTime()}</span>`;
...
function showTyping(){
  ...
  const wrap=document.getElementById('chatMessages');
  const el=document.createElement('div');
  el.className='msg msg--ai';el.id='typingIndicator';
    ...
    <div class="msg-body">
      ...
      <div class="msg-footer">
        ...
        <div class="msg-actions">
          ...
          <button class="action-btn" onclick="regenMsg(this)"><svg width="13" height="13" viewBox="0 0 24 24" fill="none"><polyline points="1 4 1 10 7 10" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/><path d="M3.51 15a9 9 0 102.13-9.36L1 10" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg> Regenerate</button>
...
function regenMsg(btn){
  ...
  setTimeout(()=>{bubble.innerHTML=renderMarkdown(DEFAULT_RESPONSE);bubble.style.opacity='1';},900);
}
...
document.addEventListener('DOMContentLoaded',()=>{
  const input=document.getElementById('messageInput');
  input.addEventListener('input',()=>{updateCounter();autoResize(input);});
